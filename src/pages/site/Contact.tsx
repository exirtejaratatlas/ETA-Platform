    ...
    <div>
      <section className="bg-surface-900 py-20">
        <div className="mx-auto max-w-4xl px-4 lg:px-8 text-center animate-fade-in-up">
          <p className="text-caption font-semibold uppercase text-copper-400 mb-3">Contact</p>
          <h1 className="text-h2 sm:text-h1 font-semibold text-white">Exir Tejarat Atlas</h1>
          <p className="mt-4 text-body-lg text-surface-300">
      ...
      <Section>
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-10">
          <div className="lg:col-span-2">
            ...
            <div className="mt-8 space-y-3">
              {["Request a Quote", "Contact Sales", "Book a Meeting"].map((label) => (
                <div key={label} className="flex items-center gap-2 text-body-sm text-surface-700">
            ...
            <div className="mt-8 space-y-5 border-t border-surface-200 pt-6">
              {offices.map((office) => (
                <div key={office.label}>
                  <p className="text-component-title sm:text-h4 font-semibold text-surface-900">{office.label}</p>
                  <div className="mt-1.5 flex items-start gap-2 text-body-sm text-surface-600">
                    <MapPin size={16} className="mt-0.5 shrink-0 text-surface-400" />
                    <span>{office.address}</span>
                  </div>
                  <div className="mt-1 flex items-center gap-2 text-body-sm text-surface-600">
          ...
          <div className="lg:col-span-3">
            {submitted ? (
              <div className="rounded-xl border border-surface-200 bg-surface-50 p-10 text-center shadow-soft">
                <CheckCircle2 size={32} className="mx-auto text-success mb-3" />
                <h3 className="text-component-title sm:text-h4 font-semibold text-surface-900">Thank you</h3>
                <p className="mt-2 text-body-sm text-surface-500">
                  We've received your message. Our team will get back to you shortly.
                </p>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="rounded-xl border border-surface-200 p-6 sm:p-8 space-y-4 shadow-soft">
                ...
                <div>
                  <label className="block text-body-sm font-medium text-surface-700 mb-1.5">Area of Interest</label>
                  <select
                    name="interest"
                    className="w-full h-9 rounded-lg border border-surface-300 bg-white px-3 text-body-sm text-surface-900 focus:border-copper-500 focus:ring-2 focus:ring-copper-500/20 focus:outline-none"
                  >
                    {interests.map((item) => (
                ...
                <div>
                  <label className="block text-body-sm font-medium text-surface-700 mb-1.5">Message</label>
                  <textarea
                    name="message"
                    rows={4}
                    required
                    placeholder="Tell us about your procurement or engineering need..."
                    className="w-full rounded-lg border border-surface-300 bg-white px-3 py-2 text-body-sm text-surface-900 placeholder:text-surface-400 focus:border-copper-500 focus:ring-2 focus:ring-copper-500/20 focus:outline-none"
                  />
                </div>
